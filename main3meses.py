...
def run_main_program():
    ...
    # Dica para resolver o problema das semanas duplicadas https://stackoverflow.com/questions/35403752/pandas-sum-over-duplicated-indices-with-sum
        ...
        def train_test_split(df):
            ...
            split_index = int(len(df_sorted) * 0.95)  # porcentagem para trenamento 0.9 = 90%
        ...
        if option:
            #periodo = st.slider("Semanas a serem removidas", 0, 2)
            #if periodo != 0:
            ...
            if option2:
                st.write("Diferenças entre os valores(sem média móvel) e as previsões:")
                st.write(errors)
            ...
            errors = (test_data['QUANT'] - forecast_table['Previsão média'])/test_data['QUANT']*100
            if option2:
                st.write("Diferenças entre os valores(sem média móvel) e as previsões em porcentagem")
                st.write(errors)
